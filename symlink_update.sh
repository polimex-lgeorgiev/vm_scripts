...
usage() {
    ...
    echo
    echo "Options:"
    echo "  -u  Run the script as a specific user (default: odoo19)"
    echo "  -h, --help  Show help information"
    echo
    echo "This script creates symlinks for all subfolders containing a __manifest__.py file."
    echo "Default source folder: /opt/odoo19/custom-addons"
    echo "Default destination folder: /opt/odoo19/addons"
    exit 1
}
...
src_folder="${1:-/opt/odoo19/custom-addons}"
dest_folder="${2:-/opt/odoo19/addons}"
...
sudo -u "$user" find "$src_folder" -type f -name "__manifest__.py" -exec dirname {} \; | while read -r folder; do
    ...
    if sudo -u "$user" [ -L "$symlink_target" ] && [ "$(sudo -u "$user" readlink -f "$symlink_target")" == "$(sudo -u "$user" readlink -f "$folder")" ]; then
        ...
        (sudo -u "$user" ln -s "$folder" "$symlink_target" && echo "Created symlink: $symlink_target -> $folder") || echo "Failed to create symlink: $symlink_target"
    fi
done
