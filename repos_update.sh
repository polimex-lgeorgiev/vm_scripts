...
if [ $# -eq 1 ]; then
    if [ $1 == "-h" ]; then
        ...
        echo ""
        echo "Arguments:"
        echo "  root_dir (optional) - The directory where the script will search for Git repositories. If not provided, the default value '/opt/odoo18/custom-addons' will be used. If this directory does not exist or there are multiple directories in /opt, the script will prompt the user to choose one."
        echo ""
        echo "Options:"
    ...
    root_dir="/opt/odoo18/custom-addons"
fi
